...
const HUMANITIES_SECTIONS = {
  italiano: {
    title: "Letteratura e Linguaggi (Italiano)",
    icon: "BookOpen",
    desc: "Un viaggio attraverso le maschere di Pirandello, il panismo di D'Annunzio e la purezza della parola di Ungaretti.",
    items: [
      {
        name: "Gabriele D'Annunzio",
        period: "1863 – 1938",
        work: "L'Estetismo e il Piacere",
        desc: "D'Annunzio è l'incarnazione del poeta-vate. La sua vita e le sue opere sono un inno all'estetismo, alla ricerca della bellezza suprema e alla fusione con la natura (panismo).\n\nCuriosità: Fu un pioniere del marketing moderno, inventando nomi di marchi celebri come 'La Rinascente' e 'Saiwa'.",
      ...
      },
      {
        ...
        period: "1888 – 1970",
        work: "L'Ermetismo e il Porto Sepolto",
        desc: "La poesia di Ungaretti nasce dal dolore della trincea. La sua rivoluzione consiste nello scavare la parola fino a renderla pura ed essenziale, eliminando ogni decoro superfluo.\n\nCuriosità: Scriveva i suoi versi su pezzi di carta di fortuna, cartoline e involucri di cartucce mentre era al fronte durante la Grande Guerra.",
      ...
      },
      {
        ...
        period: "1867 – 1936",
        work: "L'Umorismo e le Maschere",
        desc: "Pirandello esplora la frammentazione dell'io. Ogni individuo indossa una maschera per adattarsi alla società, perdendo la propria vera identità in un gioco di specchi infinito.\n\nCuriosità: Il suo concetto di 'maschera' è incredibilmente attuale se applicato ai profili social moderni, dove mostriamo solo una versione costruita di noi stessi.",
...
const PROFESSIONAL_SECTIONS = {
  informatica: {
    ...
    items: [
      { name: "C#", topic: "Software Development", desc: "Sviluppo di applicazioni desktop e web robuste utilizzando il linguaggio C# e il framework .NET.", image: "/img/os.png" },
      { name: "SQL", topic: "Database Management", desc: "Progettazione e gestione di database relazionali attraverso il linguaggio SQL per l'integrità dei dati.", image: "/img/database.png" },
      { name: "Ajax e Webservices", topic: "Web Technologies", desc: "Implementazione di comunicazioni asincrone e integrazione di servizi web per interfacce dinamiche.", image: "/img/htmljs.png" },
      { name: "JSON e XML", topic: "Data Interchange", desc: "Utilizzo di formati standard per lo scambio di dati tra applicazioni e sistemi diversi.", image: "/img/javascript.png" }
    ]
  },
  sistemi: {
    ...
    items: [
      { name: "Crittografia", topic: "Security", desc: "Protezione dell'informazione attraverso algoritmi di cifratura simmetrica e asimmetrica.", image: "/img/crittografia.png" },
      { name: "Wireshark", topic: "Network Analysis", desc: "Analisi dei pacchetti e troubleshooting delle reti per identificare problemi di traffico e sicurezza.", image: "/img/isoosi.png" },
      { name: "Packet Tracer", topic: "Simulation", desc: "Simulazione di reti complesse per il test di configurazioni router e switch Cisco.", image: "/img/cisco.png" },
      { name: "Modello ISO/OSI", topic: "Protocols", desc: "Studio dei 7 livelli di astrazione che permettono la comunicazione tra sistemi eterogenei.", image: "/img/isoosi.png" },
      { name: "Routing", topic: "Connectivity", desc: "Configurazione di algoritmi e tabelle di instradamento per la connessione tra reti diverse.", image: "/img/cisco.png" },
      { name: "Reti LAN", topic: "Local Infrastructure", desc: "Progettazione e realizzazione di infrastrutture di rete locale cablate e wireless.", image: "/img/isoosi.png" }
    ]
  },
  ia: {
    ...
    items: [
      { name: "Reti Neurali", topic: "AI Fundamentals", desc: "Modelli computazionali ispirati al cervello umano per l'apprendimento automatico.", image: "/img/neural.png" },
      { name: "Reti Convuluzionali", topic: "Computer Vision", desc: "Architetture specializzate nel riconoscimento di pattern in immagini e video.", image: "/img/neural.png" },
      { name: "Colab", topic: "Development Environment", desc: "Utilizzo di Google Colab per lo sviluppo e l'addestramento di modelli di machine learning in cloud.", image: "/img/AI-generativa.png" }
    ]
  },
  tpsit: {
    title: "Tipsit",
    ...
    items: [
      { name: "Concorrenza-Android Studio e Sistemi Operativi", topic: "Concurrent Programming", desc: "Gestione dei processi e dei thread per l'esecuzione simultanea di task in ambiente Android e desktop.", image: "/img/concorrenza.png" },
      { name: "C", topic: "Low-level Programming", desc: "Programmazione di sistema a basso livello per la gestione diretta delle risorse hardware.", image: "/img/os.png" }
    ]
  },
  gpoi: {
    title: "GPOI",
    ...
    items: [
      { name: "Busta Paga", topic: "HR Management", desc: "Analisi e comprensione dei documenti retributivi e della normativa del lavoro.", image: "/img/bustapaga.png" },
      { name: "RACI", topic: "Responsibility Mapping", desc: "Matrice per l'assegnazione delle responsabilità all'interno di un team di progetto.", image: "/img/azienda.jpg" },
      { name: "Gantt", topic: "Scheduling", desc: "Strumento per la pianificazione temporale delle attività e delle milestone di progetto.", image: "/img/costiricavi.jpg" },
      { name: "Business Plan", topic: "Entrepreneurship", desc: "Documento programmatico per la valutazione della fattibilità economica di un'idea di business.", image: "/img/azienda.jpg" },
      { name: "Project Management", topic: "Coordination", desc: "Applicazione di tecniche e strumenti per guidare un progetto verso il successo.", image: "/img/azienda.jpg" }
    ]
  }
};
                  ...
                  <section key={key} className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {section.items.map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className="bg-[#121212] border border-[#E0D8D0]/10 rounded-2xl overflow-hidden hover:border-[#E0D8D0]/30 transition-all duration-300">
                            <div className="p-6 space-y-4">
                              <div className="flex justify-between items-start">
                                <div>
                                  <h3 className="text-lg font-semibold text-[#E0D8D0]">{item.name}</h3>
                                  <span className="text-[10px] font-bold uppercase tracking-widest text-[#E0D8D0]/40">{item.topic}</span>
                                </div>
                                <button 
                                  onClick={() => toggleItem(itemId)}
                                  className="p-2 rounded-lg bg-[#E0D8D0]/5 text-[#E0D8D0] hover:bg-[#E0D8D0]/10 transition-colors"
                                >
                              ...
                              {isExpanded && (
                                <div className="space-y-4 animate-fade-in">
                                  <div className="aspect-video rounded-xl overflow-hidden border border-[#E0D8D0]/10">
                                    <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                                  </div>
                                  <p className="text-sm text-[#BDB5AD] leading-relaxed">
                                    {item.desc}
                                  </p>
                                </div>
                              )}
                              ...
                              <button 
                                onClick={() => toggleItem(itemId)}
                                className="text-xs font-semibold text-[#E0D8D0]/60 hover:text-[#E0D8D0] transition-colors flex items-center gap-1"
                              >
                                {isExpanded ? "Chiudi" : "Leggi approfondimento tecnico"}
                              </button>
                            </div>
                          </div>
                        );
